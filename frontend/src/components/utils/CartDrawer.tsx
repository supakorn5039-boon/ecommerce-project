...
export default function CartDrawer(): React.ReactElement {
  const { items, isCartOpen, closeCart, removeFromCart, clearCart, updateQuantity } = useCartStore();
      ...
      >
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(100%-200px)] space-y-4">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.id} className="flex items-start gap-4 border rounded-lg p-3 dark:border-gray-700">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="ml-auto text-red-500 hover:underline">
                      Remove
                    </button>
                  </div>
                </div>
        ...
        <div className="p-4 border-t dark:border-gray-700">
          ...
          <div className="flex justify-between gap-2">
            <button onClick={clearCart} className="w-1/2 bg-red-500 text-white py-2 rounded hover:bg-red-600">
              Clear
            </button>
            <button
